...
10. **Conditioning encoder must not build autograd graphs.** The conditioning encoder path (`get_style_emb`, `get_gpt_cond_latents`, `_get_speaker_embedding`) must run inside `torch.no_grad()`; outputs must be `.detach()`-ed before passing to vLLM. Resolved in commit `7d6299490b53aa30ff18c37a68cabe5c9ff8e2f3`.

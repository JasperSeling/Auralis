...
class TTS:
    ...
    def _run_sync(self, awaitable):
        """Run async work safely from synchronous APIs, including notebooks."""
        nest_asyncio.apply()
        try:
    ...
    def from_pretrained(self, model_name_or_path: str, **kwargs):
        ...
        self.tts_engine = self._run_sync(_load_model())
        ...
        if request.stream:
            ...
            def streaming_wrapper():
                for sub_request in requests:
                    ...
                    try:
                        while True:
                            chunk = self._run_sync(anext(generator))
                            yield chunk
            ...
            # Non streaming
            return self._run_sync(self._process_multiple_requests(requests))
